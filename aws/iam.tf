...
resource "aws_iam_role" "alertForwarder" {
  name = "${var.prefix}-f5-alertForwareder-role"
...
resource "aws_iam_instance_profile" "alertForwarder" {
  name = "${var.prefix}-alertForwarder"
  role = "${aws_iam_role.alertForwarder.name}"
}
