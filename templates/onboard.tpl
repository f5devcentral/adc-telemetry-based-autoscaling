...
user_status=`tmsh list auth user $admin_username`
if [[ $user_status != "" ]]; then
   response_status=`tmsh modify auth user $admin_username password $admin_password`
   echo "Response Code for setting user and password:$response_status"
fi
if [[ $user_status == "" ]]; then
   response_status=`tmsh create auth user $admin_username password $admin_password partition-access add { all-partitions { role admin } }`
   echo "Response Code for setting user and password:$response_status"
fi

CREDS="bigipuser:"$admin_password
DO_URL='${DO_URL}'
...
# Remove '_' from the hostname also remove - and replace with .
hostName=$(echo $hostName | tr -d _)
